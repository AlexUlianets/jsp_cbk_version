...
List<HashMap> res = new ArrayList<>();
    ...
    private HashMap findCurrentConditionForMap(DateTime dateTime, JSONObject elem){
        ...
        int tempC = Integer.parseInt(""+weather.get("temp_C"));
        int tempF = Integer.parseInt("" + weather.get("temp_F"));
        data.put("temp_C", tempC>0?"+"+tempC:tempC);
        data.put("temp_F", tempF>0?"+"+tempF:tempF);
    ...
    private HashMap find2PmWeatherForMap(DateTime dateTime, JSONObject elem){
        ...
        HashMap<String, Object> data = new HashMap<>();
        int tempC = Integer.parseInt(""+weather.get("tempC"));
        int tempF = Integer.parseInt(""+weather.get("tempF"));
        data.put("temp_C", tempC>0?"+"+tempC:tempC);
        data.put("temp_F", tempF>0?"+"+tempF:tempF);
        data.put("day", "day");
        data.put("img", "" + EXT_STATES.get(Integer.parseInt("" + weather.get("weatherCode"))));
